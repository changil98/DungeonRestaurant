...
public class Knight : PlayerCombatAI
{
    ...
    private bool isAttackSuspended = false;
    ...
    protected override void InitializeStates()
    {
        ...
        var skillState = new KnightSkillState(this);
    ...
    {
        if (CheckTarget() && !isAttackSuspended)
        {
    ...
    private void ApplyDamage()
    {
        ...
        cachedNearestTarget.TakeDamage(damage, DamageType.Physical, AttackType.Blunt,isCritical);
    }

    public override void SetForcedTarget(BaseCombatAI target, float duration)
    { 
        base.SetForcedTarget(target, duration);
    }


    public void SetDamageReduction(float percentage)
    {
        currentDamageReduction = percentage;
    }

    public void SetAttackSuspended(bool suspended)
    {
        isAttackSuspended = suspended;
    }
    ...
    {
        // 데미지 감소 적용
        float reducedDamage = damage * (1 - currentDamageReduction / 100f);
        ...
        base.TakeDamage(reducedDamage,damageType,attackType,isCritical);
    }
