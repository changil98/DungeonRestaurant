...
public class Berserker : PlayerCombatAI
{
    ...
    protected override void InitializeStates()
    {
        ...
        var skillState = new BerserkerSkillState(this);
    ...
    public override float CalculateFinalAttackSpeed()
    {
        float baseAttackSpeed = base.CalculateFinalAttackSpeed();

        if (isBerserkActive && currentSkill is Berserk berserk)
        {
            return baseAttackSpeed * (1 + berserk.attackSpeedIncrease);
        }
    ...
    private void ApplyDamage()
    {
        ...
        if (isBerserkActive && currentSkill is Berserk berserk)
        {
            berserk.OnDamageDealt(this, damage);
        }

        if (cachedNearestTarget.CurrentHP <= 0 && isBerserkActive)
        {
            OnEnemyKilled();
        }
    }
    ...
    {
        if(currentSkill is Berserk berserk)
        {
            berserk.OnEnemyKilled(this);
        }
    }

    public void SetBerserkState(bool active)
    {
        isBerserkActive = active;
    }
